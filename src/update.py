...
def check_for_updates(config):
    ...
    if semver.compare(update_version, app_version) == 1:
        ...
        if config.automatic_updates == True:
            send_notification(
                f"Auto Power Saver is now updating to version {update_version}.", config=config)
...
def on_check_updates(icon, item):
    ...
    subprocess.call(
        f"{get_powershell_path()} Stop-Process -name 'Auto Power Saver' && {get_powershell_path()} {get_sys_folder('TEMP')}\\autopowersaver_setup__v{update_version}.exe /VERYSILENT", shell=True)
    return True